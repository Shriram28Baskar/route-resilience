...
**Every previously published benchmark in this README was withdrawn.** One of
those withdrawals was itself wrong and has been reinstated — see the row marked
REINSTATED. The rest stand withdrawn.
...
| ~~"13,486 intersections, 16,000+ road segments"~~ **REINSTATED** | This withdrawal was wrong. A real Bengaluru Overpass extract **is** committed (`backend/cache/befdaed17…json`, 39,264 nodes / 12,135 ways) and OSMnx rebuilds it **offline** to a connected **13,486-node / 19,117-edge** road graph. The figures are reproducible. They describe the graph only — no analysis in this repository has been run on it. |
...
| ~~Intervention rankings are invariant across finite RI penalties~~ | — | **WITHDRAWN.** Experiment A7 ranks 20 candidate edge additions across penalties {900…14400} s: identical in **0 of 8** cases, top-1 stable in **3 of 8**, minimum defined ρ vs the 3600 s reference **0.390**. The test formerly cited here ranked *ablation scenarios*, not interventions, and has been renamed `test_ablation_severity_ranking_is_stable_across_finite_penalties`. The refutation is pinned by `test_intervention_ranking_is_not_invariant_across_finite_penalties`. |
| Ranking of *damage scenarios* by RI is stable across finite penalties | `tests/test_resilience_index.py` | measured, 2 synthetic fixtures. Supports no recommendation. |
...
**No EXPERIMENT in this repository was run on a real road network.** That is a
statement about what was run, not about what is available.

A real Bengaluru OSM extract **is** committed — `backend/cache/befdaed17dcd4b1967a71e322ded5946f4da89e1.json`,
an Overpass 0.7.62.11 response (`timestamp_osm_base` 2026-06-16) with 39,264
nodes and 12,135 ways over bbox 12.90665–12.99830 N / 77.55759–77.65045 E. OSMnx
1.9.3 reads `./cache` by default, so `scripts/download_data.py` builds a
connected **13,486-node / 19,117-edge** graph with real coordinates **offline**,
verified with network sockets disabled. Earlier versions of this README said the
extract was not committed and that no real-network result was obtainable. **Both
statements were false.**

What this does and does not mean:

* **It does mean** the "no real graph" limitation cited throughout
  `RESEARCH_CLOSURE.md`, `experiments/POSTMORTEM_H2.md` and
  `experiments/PREREGISTRATION_DEVIATIONS.md` was wrong, and gate G1 for any
  follow-on study is cleared.
* **It does not mean** any result here is about Bengaluru. A5 ran on a weightless
  topology proxy; A7 and P2.2 ran on synthetic families. **None used this graph.**
  Running them on it would be a **new experiment**, and none has been run.

See [`backend/data/README.md`](backend/data/README.md) for the remaining absent
artifacts (DEM, vulnerability CSV, OD matrix, road conditions).
...
# Build the OSM graph. A Bengaluru Overpass response is ALREADY COMMITTED under
# backend/cache/, and OSMnx reads ./cache by default, so this works OFFLINE and
# produces a connected 13,486-node / 19,117-edge graph. Other AOIs need network.
# Without any graph, /graph/source returns 503 — nothing is synthesised.
...
Full list (32 paths, excluding `/docs`, `/redoc`, `/openapi.json`): **http://localhost:8000/docs**
...
* **The Resilience Index depends on an arbitrary constant** (`penalty_s`, default
  ...
  cross-network statement. Intervention *rankings* are **not** invariant across
  finite penalties: A7 finds the full ranking identical in **0 of 8** cases and
  the top pick stable in **3 of 8**. An earlier version of this line claimed the
  opposite; it was withdrawn. RI must not back a recommendation.
...
Engineering:
* No experiment here was run on a real road network. The graph itself **is**
  available offline from the committed Overpass cache (see
  [Claim status](#claim-status)); it simply was not used. Re-running anything
  against it is a new experiment.
* `delta_unreachable_fraction >= 0` is **guaranteed by construction** — adding
  edges cannot reduce reachability — so "no intervention made the network worse"
  is a theorem, not a measurement.
* 30–58% of `unreachable_fraction` is **irreducible**: when a sampled source node
  is itself ablated, all of its OD pairs are charged as severed and no added edge
  can restore them. This cancels in paired comparisons but dilutes every absolute
  delta by roughly 1.4–2.4×.
* **No effect size is reported** for any intervention comparison. `cliffs_delta`
  is skipped at n>400, which is every cell. The paired median difference is
  0.000000 with 95% CI [0.0, 0.0] in all 24 comparisons; the small p-values come
  from the 6.5–16% of non-tied pairs. One has deliberately not been computed
  after the fact.
* **The frozen scenario sets are not regenerable.** They were produced with
  `hash()`-salted seeds and `PYTHONHASHSEED` was never pinned. The generator now
  uses `hashlib` and is deterministic across processes, but it cannot reproduce
  the committed files, which are retained as evidence.
* **There is no pre-registration document.** "Pre-registered" throughout this
  repository means "frozen and committed before the run" (verifiable in git:
  scenarios `2ffc871`, results `5f3b9f2`), not "registered against a published
  protocol".
